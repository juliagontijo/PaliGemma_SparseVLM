...
# How many viusal tokens should remain after each sparsifictaion step (so the final number of visual tokens is 192 or 128 or 64)
...
def attn_postprocess_topk(self_attn_weights, v_token_start, v_token_num, text_token_start, t_token_idx, layer_idx,retained_tokens):
    ...
    '''
    self_attn_weights = self_attn_weights.mean(1) # B, L[Q], L[K] avereges attention over the head dimension

    t_token_idx = t_token_idx[1] + text_token_start # gets the absolute position of the selected text token on attn weights
    relation_vis_text = self_attn_weights[:, t_token_idx , v_token_start: v_token_start+v_token_num] # B, L2, L1 # gets attention score from selected text token to each of the visual tokens

    relation_vis_text = relation_vis_text.mean(1) # B, L1 averages to get a single relevance score per visual token 

    relation_vis = relation_vis_text ######### THIS IS 'p^' ON THE PAPER!! 
    s_flag = True       # s_flag controls whether token merge is needed.

    sparse_token_list = sparse_token_dict[retained_tokens]

    # Produces boolean mask on which visual token to actually keep
    if v_token_num != 0:
